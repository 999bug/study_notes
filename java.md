...
- 第一范式：属性不可分
- 第二范式：唯一性 ，使得每一行数据唯一性 并消除数据之间的部分依赖
...
#### MySQL B+Tree索引和 Hash 索引的区别？

- hash 索引结构的特殊性，其检索效果非常高，索引的效率可以一次定位
- B+树索引需要从根节点到叶子节点，最后才能访问到叶子节点的，这样需要多次的IO访问。

**Hash 索引**《Heap、Memory引擎支持》
...
- B-Tree的关键字和记录都是存放到一起的。叶子节点可以看作是外部节点，不包含任何信息
- B+树的非叶子节点只有关键字和指向下一个结点的索引，记录只放在叶子节点中。
- 在B-Tree中，越靠近根节点查询速度越快，而B+树每个记录的查询时间基本一样
